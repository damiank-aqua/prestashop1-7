...
class Cz_Offercmsblock extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('CZ - Offer CMS Block', array(), 'Modules.OfferCmsBlock');
        $this->description = $this->trans('Adds custom information block in your store.', array(), 'Modules.OfferCmsBlock');
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                ...
                `text` text NOT NULL,
                PRIMARY KEY (`id_czoffercmsblockinfo`, `id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;'
        );
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'czoffercmsblockinfo`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'czoffercmsblockinfo_lang`');
        }
    ...
    public function processSaveOfferCmsBlock()
    {
        ...
        if (Shop::isFeatureActive() && !$czoffercmsblockinfo->id_shop) {
            $saved = true;
            $shop_ids = Shop::getShops();
            foreach ($shop_ids as $id_shop) {
                $czoffercmsblockinfo->id_shop = $id_shop;
                $saved &= $czoffercmsblockinfo->add();
            }
        } else {
			$czoffercmsblockinfo->id_shop = Shop::getContextShopID();
    ...
    {
        $fields_value = array();
        $id_czoffercmsblockinfo = 1;

        foreach (Language::getLanguages(false) as $lang) {
            $czoffercmsblockinfo = new OfferCmsBlock((int)$id_czoffercmsblockinfo);
            $fields_value['text'][(int)$lang['id_lang']] = $czoffercmsblockinfo->text[(int)$lang['id_lang']];
        }

        $fields_value['id_czoffercmsblockinfo'] = $id_czoffercmsblockinfo;
    ...
    {
        $sql = 'SELECT r.`id_czoffercmsblockinfo`, r.`id_shop`, rl.`text`
            FROM `'._DB_PREFIX_.'czoffercmsblockinfo` r
            LEFT JOIN `'._DB_PREFIX_.'czoffercmsblockinfo_lang` rl ON (r.`id_czoffercmsblockinfo` = rl.`id_czoffercmsblockinfo`)
            WHERE `id_lang` = '.(int)$this->context->language->id.' AND  `id_shop` = '.(int)$this->context->shop->id;
		
        return array(
            'czoffercmsblockinfos' => Db::getInstance()->getRow($sql),
			'image_url' => $this->context->link->getMediaLink(_MODULE_DIR_.'cz_offercmsblock/views/img'),
        );
